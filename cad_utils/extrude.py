...
from .macro import EXTRUDE_OPERATIONS, EXTENT_TYPE, N_ARGS_PLANE, N_ARGS_TRANS, N_ARGS_EXT, EXT_IDX, \
    SOL_IDX, EOS_VEC, N_ARGS_EXT_PARAM, NORM_FACTOR, N_ARGS_SKETCH, PAD_VAL
...
class CoordSystem(object):
    """Local coordinate system for sketch_inventor plane."""
...
class Extrude(object):
    """Single extrude operation with corresponding a sketch_inventor profile.
    NOTE: only support single sketch_inventor profile. Extrusion with multiple profiles is decomposed."""
    def __init__(self, profile: Profile, sketch_plane: CoordSystem,
        ...
        """
        Args:
            profile (Profile): normalized sketch_inventor profile
            sketch_plane (CoordSystem): coordinate system for sketch_inventor plane
            ...
            sketch_pos (np.array): the global 3D position of sketch_inventor starting point
            sketch_size (float): size of the sketch_inventor
        """
        self.profile = profile # normalized sketch_inventor
    ...
    def from_dict(all_stat, extrude_id, sketch_dim=256):
        ...
        Args:
            ...
            sketch_dim (int, optional): sketch_inventor normalization size. Defaults to 256.
        ...
        for i in range(len(extrude_entity["profiles"])):
            sket_id, profile_id = extrude_entity["profiles"][i]["sketch_inventor"], extrude_entity["profiles"][i]["profile"]
    ...
    def __str__(self):
        ...
        s += "\n  -sketch_inventor position: {}, sketch_inventor size: {}".format(self.sketch_pos.round(4), self.sketch_size.round(4))
...
class CADSequence(object):
    ...
    def random_transform(self):
        for item in self.seq:
            # random transform sketch_inventor
